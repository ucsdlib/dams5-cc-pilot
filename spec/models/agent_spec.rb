...
describe Agent do
  ...
  describe '#alternateLabel' do
    let(:subject) { Agent.new(label: ['Test Agent'], alternate_label:['Alternate label']) }
    it 'has a label' do
      expect(subject.label).to eq ['Test Agent']
  ...
  describe '#closeMatch' do
    let(:subject) { Agent.new(label: ['Test Agent'], close_match: 'http://test.com/closeMatch') }
    let(:subject_error) { Agent.new(label: ['Test Agent'], close_match: 'xsd:anyURI') }
  ...
  describe '#relatedMatch' do
    let(:subject) { Agent.new(label: ['Test Agent'], related_match: 'http://test.com/relatedMatch') }
    let(:subject_error) { Agent.new(label: ['Test Agent'], related_match: 'relatedMatch: xsd:anyURI') }
